  ...
  (multiple-value-bind (types functions)
    ...
    (labels ((normalize-type (type &optional require-type-func)
             ...
             (enumlike-body (elems prefix)
               (mapcar (lambda (value-expr)
                         (multiple-value-bind (name value)
                             (match-ecase value-expr
                               ((m:satisfies #'dom:element-p)
                                (assert (equal (dom:tag-name value-expr) "enum"))
                                (values (get-attribute value-expr "name")
                                        (let ((val-str (get-attribute value-expr "value"))
                                              (bitpos (get-attribute value-expr "bitpos")))
                                          (if val-str
                               ...
                               ((:synthetic-enum name value) (values name value)))
                           (assert (string= prefix name :end2 (length prefix)))
                           `(,(intern (nsubstitute #\- #\_ (subseq name (length prefix))) :keyword) ,value)))
                       (remove-if (lambda (el) (and (dom:element-p el)
                                                    (or (member (dom:tag-name el) '("comment" "unused") :test #'equal)
  ...
  (multiple-value-bind (features extensions)
    ...
    (multiple-value-bind (type-table const-table func-table feat-table ext-table enum-table) (load-registry file)
      (let ((package (or (find-package package-name) (make-package package-name)))
            (syms ()))
        (labels ((parse-require-elem (elem &optional extension-num author-tag)
                   ...
                   (mapcan (lambda (required-thing)
                             (let ((name (get-attribute required-thing "name")))
                               (match-ecase (dom:tag-name required-thing)
                                 ...
                                 ("enum"
                                  (when-let (extended (get-attribute required-thing "extends"))
                                    (let ((value (if-let (offset (get-attribute required-thing "offset"))
                                                   (* (match-ecase (get-attribute required-thing "dir")
                                                        ('nil 1)
                                                        ("-" -1))
                 ...
                 (parse-extension (elem)
                   (let ((author-tag (get-attribute elem "author")))
                     (assert author-tag)
                     (mapcan (rcurry #'parse-require-elem (parse-integer (get-attribute elem "number")) author-tag)
                             (child-elems elem)))))
          `(progn
